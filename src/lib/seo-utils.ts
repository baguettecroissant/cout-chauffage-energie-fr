...
export function generateCityMetadata(city: City, categoryLabel: string, categorySlug: string) {
    const title = `${categoryLabel} à ${city.name} (${city.zip}) — Coût d'installation et devis 2026`;
    const description = `Quel est le coût d'installation ${categoryLabel.toLowerCase().startsWith('v') ? 'de la' : categoryLabel.toLowerCase().startsWith('c') ? 'du' : 'de la'} ${categoryLabel.toLowerCase()} à ${city.name} ? Comparez les tarifs des installateurs du ${city.department_name} et obtenez votre devis gratuit.`;
    const url = `https://www.cout-chauffage-energie.fr/${categorySlug}/${city.slug}`;
    return {
        title,
        description,
