        ...
        <header className="sticky top-0 z-50 w-full border-b bg-white shadow-sm">
            <div className="container mx-auto flex h-16 md:h-20 items-center justify-between px-4 sm:px-6 lg:px-8">
                ...
                <Link href="/" className="flex items-center gap-2">
                    <span className="text-lg md:text-2xl font-bold text-zinc-800 tracking-tight">
                        Cout-Chauffage-Énergie<span className="text-orange-600">.fr</span>
                    </span>
                </Link>
                ...
                <nav className="hidden md:flex items-center gap-8">
                    {navLinks.map((link) => (
                        <Link
                            key={link.href}
                            href={link.href}
                            className="flex items-center gap-2 text-zinc-600 hover:text-orange-600 font-medium transition-colors"
                        >
                ...
                <div className="flex items-center gap-4">
                    ...
                    {/* Mobile Menu Button */}
                    <button
                        className="md:hidden p-2 text-zinc-600 hover:text-orange-600 transition-colors"
                    ...
                    <nav className="container mx-auto px-4 py-4 space-y-2">
                        {navLinks.map((link) => (
                            <Link
                                key={link.href}
                                href={link.href}
                                className="flex items-center gap-3 p-3 text-zinc-700 hover:bg-orange-50 hover:text-orange-600 rounded-lg font-medium transition-colors"
