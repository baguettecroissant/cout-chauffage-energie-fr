...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
    ...
    if (!dept) return {};
    return {
        title: `Installateurs Chauffage ${dept.name} (${dept.code}) — Devis Gratuits`,
        description: `Trouvez un installateur certifié RGE dans le ${dept.name}. Géothermie, chauffe-eau, VMC, chaudière bois : devis gratuits.`,
        alternates: { canonical: `https://www.cout-chauffage-energie.fr/annuaire/${slug}` },
    };
}
...
export default async function DepartmentPage({ params }: Props) {
    ...
    const mainCategory = CATEGORY_LIST[0]; // Default to first category
        ...
        <div className="min-h-screen bg-white">
            <header className="bg-zinc-50 border-b border-zinc-200">
                <div className="container mx-auto px-4 max-w-5xl py-3 text-sm text-zinc-500">
                    <nav className="flex items-center gap-1">
                        ...
                        <Link href="/annuaire" className="hover:text-orange-600">Annuaire</Link>
                        <ChevronRight className="h-3 w-3" />
            ...
            <section className="container mx-auto px-4 py-12 max-w-5xl">
                <h1 className="text-3xl md:text-4xl font-black text-zinc-900 mb-2">
                    Installateurs chauffage dans le {dept.name}
                </h1>
                    ...
                    {sortedCities.map(city => (
                        ...
                        >
                            ...
                            <div className="min-w-0">
                                <span className="text-zinc-700 group-hover:text-orange-600 font-medium block truncate">{city.name}</span>
